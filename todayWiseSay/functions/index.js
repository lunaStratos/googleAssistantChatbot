...
//intent name
const WELCOME_INTENT = 'Welcome Intent';
//basic intent : fallback & exit
...
app.intent(WELCOME_INTENT, (conv) => {
  ...
  const randomHouse = randomField(
    ...
    "나는 어떠한 아메리칸 드림도 보지 못하고, 아메리칸 악몽(나이트메어)을 봅니다. \n- 맬컴 엑스",
    ...
    "내가 하면 로맨스, 남이 하면 불륜. \n- 박희태, '총체적 난국'이나 '정치 9단', '남이 하면 불륜 내가 하면 로맨스' 등 수많은 관용 표현들의 창시자",
    ...
    "등락할 것입니다. \n- J. P. 모건(Morgan), 의회 청문회에서 주식시장의 전망에 관한 질문을 받고서 한 대답",
    ...
    "이것은 한 인간에게는 작은 발걸음이지만 인류에게는 커다란 도약입니다. \n- 닐 암스트롱, 1969.7.21. UTC 2:56. 달에 첫 인류의 발자국을 찍으면서",
    ...
    "인(仁)을 짓밟는 자를 '적(賊)' 이라 하고 의를 버리는 자를 '잔(殘)' 이라 합니다. 잔적한 자는 일개 필부에 불과할 뿐입니다. 주무왕(武王)이 필부인 주(紂)를 죽였다는 말은 들었으나, 임금을 시해하였다는 말은 듣지 못하였습니다. \n- 맹자, 신하가 임금을 시해할 수 있느냐는 제나라 선왕의 질문에 대한 대답. 역성혁명을 인정하는 내용이었기에 훗날 많은 반정(이라 쓰고 쿠데타라고 읽는 것들)의 근거가 된 글귀이기도 하다",
    ...
    "적과 싸우는 것은 내가 군인이기에 한 일입니다. 이런 강도와 싸우는 것도 내가 인간이기에 한 일일 뿐입니다. \n- 비슈누 쉬레스타,유명한 용병집단인 네팔의 구르카에서 현역으로 근무하다가 은퇴한 퇴역군인. 기차여행 중 40인의 강도단을 만나 돈을 뜯길 때도 조용히 있다가 강도두목이 소녀를 강간하려고 하자 열 받아서 40명 격퇴 뒤 소녀의 부모가 6천 5백 루피를 주려고 했을 때 거절했다",
    "전쟁에서 2등을 위한 자리는 없다. \n- 오마르 브래들리",
...
exports.todayWiseSaying = functions.https.onRequest(app);
